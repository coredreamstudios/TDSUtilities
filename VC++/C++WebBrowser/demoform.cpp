...
{ MSG msg; while (true)
  { BOOL res=PeekMessage(&msg,0,0,0,PM_REMOVE);
    if (!res) return;
    if (msg.message==WM_QUIT) {isquit=true; return;}
...
{ switch (msg)
  { case WM_CREATE:
    { // We'll create a browser with a scroll-bar, so we use CreateWindow with WS_VSCROLL.
      ...
      hwebf = CreateWindow(WEBFORM_CLASS,_T("http://www.wischik.com/lu/Programmer"),WS_CHILD|WS_CLIPSIBLINGS|WS_VISIBLE|WS_VSCROLL,0,0,100,100,hwnd,(HMENU)103,hInstance,0);
      //
      // We're going to surprise the user in 20 seconds with a dialog popping up.
    ...
    { KillTimer(hwnd,1);
      ...
      loaded=false; has_clicked=false; WebformGo(hwebf,0);
      while (!loaded && !isquit) {PumpMessages();Sleep(5);} 
      if (isquit) {PostQuitMessage(0);return 0;}
      ...
      while (!has_clicked && !isquit) {PumpMessages();Sleep(5);} // Now it's been clicked!
      while (!loaded && !isquit) {PumpMessages();Sleep(5);} 
      //
      ...
      if (sub!=0 && _tcscmp(sub,_T("Ok"))==0) MessageBox(hwnd,tx?tx:_T(""),cx?cx:_T(""),MB_OK);
      //
      ...
      loaded=false; WebformGo(hwebf,_T("http://www.wischik.com/lu/Programmer"));
      while (!loaded && !isquit) {PumpMessages();Sleep(5);}
      if (isquit) {PostQuitMessage(0);return 0;}
    ...
    case WM_PAINT:
    { PAINTSTRUCT ps; BeginPaint(hwnd,&ps); FillRect(ps.hdc,&ps.rcPaint,(HBRUSH)GetStockObject(WHITE_BRUSH));
      EndPaint(hwnd,&ps);
      return 0;
    } 
    case WM_COMMAND:
    { int id=LOWORD(wParam), code=HIWORD(wParam);
      ...
      { if (!has_clicked) {has_clicked=true; break;} // see the dialog code above.
        //
    ...
    { WPARAM key=wParam; // 0xA6=VK_BROWSER_BACK, 0xA7=VK_BROWSER_FORWARD
      ...
      { // If focus is in a text-element, then Backspace will delete characters.
        ...
        if (focus!=0)
        { IHTMLTextElement *text=0; focus->QueryInterface(IID_IHTMLInputTextElement,(void**)&text);
